...
void write_makefile_featurerich() {
  ...
  char comment_autoinclude_incdirs[] =
      "# automatically add the -I onto each include directory";
  char cflags_debug[] = "CFLAGS_DEBUG=-Wall -Wextra -g $(foreach "
                        "D,$(INCDIRS),-I$(D)) $(OPT_DEBUG) $(DEPFLAGS)";
  char cflags_release[] = "CFLAGS_RELEASE=-Wall -Wextra $(foreach "
                          "D,$(INCDIRS),-I$(D)) $(OPT_RELEASE) $(DEPFLAGS)";
  ...
  char comment_regular_expression_completion_c_sources[] =
      ...
      "(wildcard)";
  char cfiles[] = "CFILES=$(foreach D,$(CODEDIRS),$(wildcard $(D)/*.c))";
  char comment_regex_replacement[] = "# regular expression replacement";
  char objs[] = "OBJECTS=$(patsubst %.c,%.o,$(CFILES))";
  char depfiles[] = "DEPFILES=$(patsubst %.c,%.d,$(CFILES))";
  char build_all[] = "all: $(BINARY)";
  char build_binaries[] = "$(BINARY): $(OBJECTS)";
  char build_binaries_recipe[] = "$(CC) -o $@ $^ $(LDFLAGS)";
  char comment_c_dependency[] =
      "# only want the .c file dependency here, thus $< instead of $^.";
  char build_objs[] = "%.o:%.c";
  char build_objs_recipe_debug[] = "$(CC) $(CFLAGS_DEBUG) -c -o $@ $<";
  // char build_objs_recipe_release[] = "# $(CC) $(CFLAGS_RELEASE) -c -o $@ $<";
  char cleanup[] = "clean:";
  ...
  char diff[] = "diff:";
  char diff_recipe_one[] =
      "$(info The status of the repository, and the volume "
      "of per-file changes:)";
  ...
  if ((fptr = fopen(filename, "w")) == NULL) {
    ...
    // man fopen
    fprintf(stderr,
            "ERROR: opening file failed! Filename: %s | error code: %s!\n",
